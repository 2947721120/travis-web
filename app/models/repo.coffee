...
if Config.useV3API
  Repo = Model.extend
    ...
    lastBuildState: Ember.computed.oneWay('defaultBranch.lastBuild.state')
    lastBuildNumber: Ember.computed.oneWay('defaultBranch.lastBuild.number')
    lastBuildStartedAt: Ember.computed.oneWay('defaultBranch.lastBuild.startedAt')
    lastBuildDuration: Ember.computed.oneWay('defaultBranch.lastBuild.duration')
  ...
  Repo = Model.extend
    ...
    lastBuild: (->
      if id = @get('lastBuildId')
        if @store.hasRecordForId('build', id)
          @store.peekRecord('build', id)
        else
          @store.findRecord('build', id).then =>
            @notifyPropertyChange('lastBuild')
